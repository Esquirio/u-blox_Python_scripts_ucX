...
def send_data_via_bluetooth_classic(sock, data):
    ...
    try:
        ...
        print(Fore.GREEN + f"Sending data: {data.decode()}")
        print(Fore.GREEN + f"Sending hex data: {hex_data}")
        sock.send(data)
...
async def write_gatt_char_ble(client, data, service_uuid, characteristic_uuid):
    ...
    if service_uuid and characteristic_uuid:
        ...
        hex_data = data_to_hex(data)
        print(Fore.YELLOW + f"Writing data: {data}")
        print(Fore.GREEN + f"Writing data: {hex_data}")
    else:
        print(Fore.RED + "Service or characteristic not found. Cannot send data.")

async def read_gatt_char_value(client, service_uuid, characteristic_uuid):
    """
    Finds the matching service and characteristic and reads data from the Bluetooth device.

    :param client: The BleakClient instance.
    :param service_uuid: The prefix of the service UUID to match.
    :param characteristic_uuid: The prefix of the characteristic UUID to match.
    """
    service_uuid, characteristic_uuid = await find_service_and_characteristic(client, service_uuid, characteristic_uuid)
    ...
    if service_uuid and characteristic_uuid:
        if client.is_connected:
            ...
            print(Fore.YELLOW + f"Data read: {data.decode()}")
            print(Fore.GREEN + f"Data read in hex: {hex_data}")
    else:
        print(Fore.RED + "Service or characteristic not found. Cannot read data.")

async def main_ble(target_address, service_uuid, characteristic_uuid):
    """
...
if __name__ == "__main__":
    ...
    if args.type == "classic":
        ...
        asyncio.run(main_ble(target_address, service_uuid, characteristic_uuid))
